...
Taking a look at the resulting tree we can see that the median estimates is not so different from previous results, even if we have some very wide CIs (but it was expected since we have not use any lower or uppper boundary). **NICE!**
