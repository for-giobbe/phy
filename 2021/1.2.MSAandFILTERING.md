...
Now run gblocks on all our alignments with a simple bash loop:

```
for i in $( find ./Analyses/1-to-1_Alignments/ -type f \( -iname \*.mafft -o -iname \*.mafftdash -o -iname \*aln \)); do /Users/jacopomartelossi/Desktop/phylogenetic_software/Gblocks_0.91b/Gblocks "$i" -t=p -b5=a 2>&1 | "$i".gb.log; done
```
