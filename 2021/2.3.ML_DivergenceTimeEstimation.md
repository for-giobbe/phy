...
Now open the nexus file with figtree and take a look at the resulting tree (remember to display also the node ages)...

..As you can see there is a very short branch between *A.granulata* and other Mollusks. This may be due to 3 variables: (1) **Low taxon sampling**; (2) **Wrong topology** and (3) a true **fast radiation** of Mollusks between the Precambrian and the Cambrian. 

IQ-TREE also implements the possibility to calculate CF of nodes. We can do this while using less calibration points, so we can better understand what our data are telling us (and eventually highlights some conflicts with previously used calibrations point). Now, remove all priors except the mollusk one and run:
...
Taking a look at the resulting tree we can see that the median estimates is not so different from previous results, even if we have some very wide CFs (but whis was expected wince we have not use any lower or uppper boundary). NICE!
