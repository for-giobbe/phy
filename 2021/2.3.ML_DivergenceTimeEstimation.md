...
iqtree -s Analyses/IQ-TREE/My_Concat.fa --date Data/My_Calibrations.txt -spp Analyses/IQ-TREE/spp_MergedPartitions.best_scheme.nex --date-tip 0 -o "H_robusta" -te Analyses/IQ-TREE/specie.tree.treefile --prefix Analyses/IQ-TREE/Time_Tree/My_DivergenceTime
```

Results are almost the same as describe before, beside the presence of a timetree.nexus file. The nexus format is especially used in Bayesian inference where it can describe all informations that this type of analysis brings with them (*e.g* confidence intervals).

Now open the nexus file with figtree and take a look at the resulting tree (display also the node ages). As we can see there is 
