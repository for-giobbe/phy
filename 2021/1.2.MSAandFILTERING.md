...
Now run gblocks on all our alignments with a simple bash loop (we will also redirect stdout and stderr to a log file):

```
for i in $( find ./Analyses/1-to-1_Alignments/ -type f \( -iname \*.mafft -o -iname \*.mafftdash -o -iname \*aln \)); do varOG=$(echo "$i" | rev | cut -d"/" -f1 | rev); /Users/jacopomartelossi/Desktop/phylogenetic_software/Gblocks_0.91b/Gblocks "$i" -t=p -b5=a 2>&1 | tee "$i".gb.log; done
```

Now, we can roughly summarize results of gblock with a simple bash script:

```
for i in $( ls Analyses/1-to-1_Alignments/ | grep "mafft\|mafft_dash\|t_cofee"); do varResults=$( cat ./Analyses/1-to-1_Alignments/"$i"/*.log | grep "Gblocks alignment" | cut -d" " -f6 | sed 's/(//g' | awk '{ total += $1 } END { print total/NR }'); echo -e "$i\t$varResults"; done
```

 * For which set of alignments Gblock conserved on evarege more positions?
 * Inside this set, try to find alignments with more ambiguous positions.
