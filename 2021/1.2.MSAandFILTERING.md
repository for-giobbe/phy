...
In this tutorial, we will use the most popular tools for multiple sequence alignment:
 ...
 ```
mkdir -p Analyses/1-to-1_Alignments/{mafft,t_coffee,trimAi,gblock,psi_coffee}
 ```
 ### 1. Mafft
 ...
 * the **.html** file mirrors the .aln file, it's just easyer to open by double clicking on it.
 ...
 ### 3. The tradeoff between speed and accuracy: [PSI-Coffe](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4987888/)

One of the most accurate method (IMHO) to rely on. This approach is quite useful when dealing with distantly related proteins and can be a game-changer when accurate alignments are necessary, for example for inferences on selection regimes. At its basis there is a combination of homology extension with a consistency based alignment. Homology extension is performed with Position Specific Iterative (PSI) BLAST searches against a choice of redundant and non-redundant databases.
